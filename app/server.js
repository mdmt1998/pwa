...
function getForecast(req, resp) {
  ...
  const url = `${BASE_URL}?key=${API_KEY}&lat=${location.split(',')[0]}&lon=${location.split(',')[1]}`;
  fetch(url).then((resp) => {
    if (resp.status !== 200) {
    ...
    setTimeout(() => {
      const forecast = {
        ...
        currently: {
          ...
          humidity: data.data[0].precip,
          windSpeed: data.data[0].wind_spd,
          windBearing: data.data[0].wind_spd.wind_dir,
        },
        daily: {
          data: []
        }
      ...
      for(var i = 1; i < data.data.length; i++) {
        ...
        forecast.daily.data.push({
          ...
          sunriseTime: element.sunrise_ts,
          sunsetTime: element.sunset_ts,
          temperatureHigh: element.high_temp,
          temperatureLow: element.low_temp,
        });
      }
